...
DrawablePtr createPointCloud(const float *pX, const float *pY, const float *pZ,
  ...
  for (unsigned int uiY = uiYmin; uiY < uiYmax /*p_uiImageHeight*/;
       uiY++ /*=iStride*/) {
    for (unsigned int uiX = uiXmin; uiX < uiXmax /*p_uiImageWidth*/;
         uiX++ /*=iStride*/) {
      uiLoop = uiY * p_uiImageWidth + uiX;

      if (std::isnormal(pX[uiLoop]) == 0 || std::isnormal(pY[uiLoop]) == 0 ||
          std::isnormal(pZ[uiLoop]) == 0)
        continue;

      if (0.0f != pX[uiLoop] && 0.0f != pY[uiLoop] && RANGE_MIN <= pZ[uiLoop] &&
          RANGE_MAX >= pZ[uiLoop]) {
        uiCount++;
  ...
  {
    osg::ref_ptr<osgUtil::DelaunayTriangulator> poTriangulator =
  ...
  //! <LI>Build Z Color Map
  {
    ...
    for (unsigned int uiLoop = 0; uiLoop < paroVertices->size(); uiLoop++) {
      toColor(((*paroVertices)[uiLoop][2] - fMin) / (fMax - fMin),
              &((*paroColors)[uiLoop][0]), &((*paroColors)[uiLoop][1]),
              &((*paroColors)[uiLoop][2]));
    }
  }
...
struct CPlop {
  ...
  //!!!
  bool operator()(struct CPlop p_oI, struct CPlop p_oJ) {
    if (p_oI.fX < p_oJ.fX)
      return true;
    else if (p_oI.fX > p_oJ.fX)
      return false;
    else if (p_oI.fY < p_oJ.fY)
      return true;
    else if (p_oI.fY > p_oJ.fY)
      return false;
    else
      return (p_oI.fZ < p_oJ.fZ);
  }
} myPlop;

DrawablePtr CreateTexturedPointCloud(const float *p_pfX, const float *p_pfY,
                                     const float *p_pfZ,
  ...
  for (unsigned int uiY = uiYmin; uiY < uiYmax; uiY += iStride) {
    for (unsigned int uiX = uiXmin; uiX < uiXmax; uiX += iStride) {
      ...
      if (std::isnormal(p_pfX[uiLoop]) == 0 ||
          std::isnormal(p_pfY[uiLoop]) == 0 ||
          std::isnormal(p_pfZ[uiLoop]) == 0)
        continue;

      if (0.0f != p_pfX[uiLoop] && 0.0f != p_pfY[uiLoop] &&
          RANGE_MIN <= p_pfZ[uiLoop] && RANGE_MAX >= p_pfZ[uiLoop]) {
        uiCount++;
        paroVertices->push_back(
            osg::Vec3f(p_pfX[uiLoop], p_pfY[uiLoop], p_pfZ[uiLoop]));
        paroNormals->push_back(osg::Vec3f(0.0f, 0.0f, 1.0f));
        paroColors->push_back(osg::Vec4f(1.0f, 1.0f, 1.0f, 1.0f));
  ...
  //! <LI>Compute Delaunay Triangulation
  {
    std::cerr << "OSG: Epipolar Point Count Before Delaunay : " << uiCount
              << std::endl;
    ...
    std::cerr << "OSG: Epipolar Point Count After Delaunay : "
              << paroVertices->size() << std::endl;
  }
  ...
  //! <LI>Build Z Color Map
  {
    ...
    for (unsigned int uiVerticesLoop = 0; uiVerticesLoop < paroVertices->size();
         uiVerticesLoop++) {
      (*paroColors)[uiVerticesLoop][0] =
          (aroPlop[uiVerticesLoop].ucRed / static_cast<float>(0xff));
      (*paroColors)[uiVerticesLoop][1] =
          (aroPlop[uiVerticesLoop].ucGreen / static_cast<float>(0xff));
      (*paroColors)[uiVerticesLoop][2] =
          (aroPlop[uiVerticesLoop].ucBlue / static_cast<float>(0xff));
    }
  }
  //! <LI>Attach Color Array to Geometric Node.
  poGeometry->setColorBinding(osg::Geometry::BIND_PER_VERTEX);
