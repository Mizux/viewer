...
ImageViewer2D::ImageViewer2D(QWidget* parent) : ViewerQT(parent), _rootNode(0) {
  _setupWidget();
}
...
bool ImageViewer2D::_addImage(const QImage& image) {
  ...
  osg::ref_ptr<osg::Texture2D> texture2dPtr = new osg::Texture2D;
  {
    ...
    texture2dPtr->setFilter(osg::Texture2D::MIN_FILTER,
                            osg::Texture2D::NEAREST);
    texture2dPtr->setFilter(osg::Texture2D::MAG_FILTER,
                            osg::Texture2D::NEAREST);

    //! Clamp Texture (no tiling).
    texture2dPtr->setWrap(osg::Texture2D::WRAP_S, osg::Texture2D::CLAMP);
    texture2dPtr->setWrap(osg::Texture2D::WRAP_T, osg::Texture2D::CLAMP);
  ...
  {
    stateSetPtr->setTextureAttributeAndModes(0, texture2dPtr.get(),
  ...
  osg::ref_ptr<osg::PositionAttitudeTransform> patPtr =
  ...
  if (QImage::Format_RGB16 == image.format()) {
    unsigned char* buffer =
        new unsigned char[image.width() * image.height() * 2];
    ::memcpy(buffer, image.bits(), image.width() * image.height() * 2);

    imagePtr->setImage(image.width(), image.height(), 0, GL_RGB, GL_RGB,
                       GL_UNSIGNED_SHORT_5_6_5, buffer,
                       osg::Image::USE_NEW_DELETE);
  } else if (QImage::Format_RGB888 == image.format()) {
    unsigned char* buffer =
        new unsigned char[image.width() * image.height() * 4];
    ::memcpy(buffer, image.bits(), image.width() * image.height() * 4);

    imagePtr->setImage(image.width(), image.height(), 0, GL_RGB, GL_RGB,
                       GL_UNSIGNED_BYTE, buffer, osg::Image::USE_NEW_DELETE);
  } else if (QImage::Format_ARGB32 == image.format()) {
    unsigned char* buffer =
        new unsigned char[image.width() * image.height() * 4];
    ::memcpy(buffer, image.bits(), image.width() * image.height() * 4);

    imagePtr->setImage(image.width(), image.height(), 0, GL_RGBA, GL_RGBA,
