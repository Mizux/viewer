...
AdapterWidget::AdapterWidget(QWidget* parent, const QString& name,
...
void AdapterWidget::keyPressEvent(QKeyEvent* event) {
  _gw->getEventQueue()->keyPress(
      static_cast<char>(*(event->text().toAscii().data())));
}
...
void AdapterWidget::mouseMoveEvent(QMouseEvent* event) {
  _gw->getEventQueue()->mouseMotion(static_cast<float>(event->x()),
